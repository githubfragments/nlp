...
def linear(input, output_size, bias, scope):
    with tf.variable_scope(scope):
        return _linear(input, output_size, bias)
...
def init0(shape, dtype, partition_info=None):
  return tf.zeros(*shape, dtype=dtype)
  
def init1(shape, dtype, partition_info=None):
...
class HyperLnLSTMCell(rnn_cell.RNNCell):
  ...
  def __init__(self, num_units, forget_bias=1.0, input_size=None,
    ...
    self.hyper_cell = rnn_cell.BasicLSTMCell(hyper_num_units, state_is_tuple=state_is_tuple)
    #self.hyper_cell = tf.nn.rnn_cell.LSTMCell(hyper_num_units, state_is_tuple=state_is_tuple)
  ...
  def hyper_norm(self, layer, dimensions, scope="hyper"):
    with tf.variable_scope(scope):
      zw = linear(self.hyper_output, self.hyper_embedding_size, False, scope=scope+ "z")
      alpha = linear(zw, dimensions, False, scope=scope+ "alpha")
  ...
  def __call__(self, inputs, state, scope=None):
    ...
    with vs.variable_scope(scope or type(self).__name__):
      ...
      self.hyper_state = tf.concat([total_h[:, self._num_units:], total_c[:, self._num_units:]], 1)
      hyper_input = tf.concat([inputs, h], 1)
      ...
      input_below_ = linear([inputs], 4 * self._num_units, False, scope="out_1")
      input_below_ = self.hyper_norm(input_below_, 4 * self._num_units, scope="hyper_x")
      
      state_below_ = linear([h], 4 * self._num_units, False, scope="out_2")
...
#         b3 = vs.get_variable("b3", initializer=tf.zeros([self._num_units]), dtype=tf.float32)
        ...
        s1 = vs.get_variable("s1", initializer=init1, shape=[4 * self._num_units], dtype=tf.float32)
        s2 = vs.get_variable("s2", initializer=init1, shape=[4 * self._num_units], dtype=tf.float32)
        #s3 = vs.get_variable("s3", initializer=init1, shape=[self._num_units], dtype=tf.float32)
 
        b1 = vs.get_variable("b1", initializer=init0, shape=[4 * self._num_units], dtype=tf.float32)
        b2 = vs.get_variable("b2", initializer=init0, shape=[4 * self._num_units], dtype=tf.float32)
        #b3 = vs.get_variable("b3", initializer=init0, shape=[self._num_units], dtype=tf.float32)
